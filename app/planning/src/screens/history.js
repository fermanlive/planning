...
async componentDidMount(){
  ...
  let Periods = await ReadPeriod(idUser,null);
  Periods = Periods.status? Periods.message: [];
  Periods = Periods.length? Periods: [Periods];

  for (let index = 0; index < Periods.length; index++) {
    Periods[index].key=index;
    Periods[index].label=Periods[index].name;
  }

  this.setState({selectPeriods:  Periods});
...
this.setState({
  ...
  async loadMonth(IdPeriod){ 
    this.setBusyIndicator(true, '');
    this.setPeriod(this.state.idUser,IdPeriod);
    this.setBalance(this.state.idUser,IdPeriod);
          ...
          <Text style={[text.TravelInfoTitle, text.Regular, text.TBlack]}>
            ...
            <ModalSelector
                ...
                onChange={(option)=>{ 
                  ...
                  this.loadMonth(option.idperiod),
